...
export const logout = asyncHandler(async (req, res) => {
  await authService.logout(req.user!.id, req.user!.sessionId, getRequestMeta(req));
...
export const requestOtp = asyncHandler(async (req, res) => {
  const result = await authService.requestOtp(req.body.phone, req.body.purpose, getRequestMeta(req));
  return sendSuccess(res, result, "OTP sent");
});

export const verifyOtp = asyncHandler(async (req, res) => {
  const result = await authService.verifyOtp(req.body, getRequestMeta(req));
  return sendSuccess(res, result, "OTP verified");
});

export const requestEmailVerification = asyncHandler(async (req, res) => {
  const result = await authService.requestEmailVerification(req.body.email, getRequestMeta(req));
  return sendSuccess(res, result, "Verification email sent");
});

export const verifyEmail = asyncHandler(async (req, res) => {
  const result = await authService.verifyEmail(req.body.token, getRequestMeta(req));
