...
import { ApiRolePayload, mapApiRole } from '@utils/api-mappers';
...
export class RoleService {
            ...
            return runResourceLoader(
                async () => {
                    ...
                    const [rolesResponse, permissionsResponse] = await Promise.all([
                        ...
                        this.httpClient.get<
                            { id: string; action: string; subject: string; code: string }[]
    ...
    async updateRolePermissions(roleId: string, permissionIds: string[]): Promise<Role | null> {
        try {
            const response = await this.httpClient.put<ApiRolePayload>(
