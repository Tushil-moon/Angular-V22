      ...
      @if (isLoading()) {
        ...
        <div class="kanban-board" cdkDropListGroup>
          @for (column of columns(); track column.stage) {
            <div class="kanban-column">
              <div class="kanban-column-header">
                <app-badge [variant]="stageBadgeVariant(column.stage)">{{ formatStage(column.stage) }}</app-badge>
              ...
              >
                @for (deal of column.deals; track deal.id) {
                  <div class="kanban-card" cdkDrag [cdkDragDisabled]="!canManage()">
                    <p class="kanban-card-title">{{ deal.title }}</p>
                    <p class="kanban-card-value">{{ formatValue(deal.value, deal.currency) }}</p>
                    @if (deal.contact?.fullName) {
                      <p class="kanban-card-meta">{{ deal.contact.fullName }}</p>
                    }
                  </div>
                }
              </div>
