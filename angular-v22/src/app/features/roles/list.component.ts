              ...
              @for (role of filteredRoles(); track role.id) {
                <div class="role-card space-y-4">
                  <div class="flex items-start justify-between gap-3">
                    ...
                    <span [class]="role.isActive ? 'badge badge-success' : 'badge badge-danger'">
                      {{ role.isActive ? 'Active' : 'Inactive' }}
                    </span>
                  </div>
                  @if (role.permissions?.length) {
                    <div class="flex flex-wrap gap-1.5">
                      @for (permission of role.permissions!.slice(0, 4); track permission.id) {
                        <span class="badge badge-secondary text-[10px]">{{ permission.code }}</span>
                      }
