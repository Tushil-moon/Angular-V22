      ...
      <app-card [fill]="true">
        <app-card-header [row]="true">
          <div class="min-w-0 space-y-1">
            ...
            <app-card-description>{{ totalCompanies() }} total companies</app-card-description>
          </div>
            ...
            @for (company of companies(); track company.id) {
              <app-flex-table-row>
                <app-flex-table-cell column="name">
                  <div class="min-w-0">
                    <p class="truncate font-medium text-foreground">{{ company.name }}</p>
                    @if (company.website) {
                      <p class="truncate text-xs text-muted-foreground">{{ company.website }}</p>
                    }
                  </div>
                </app-flex-table-cell>
                <app-flex-table-cell column="domain">
                  <span class="truncate text-muted-foreground">{{ company.domain || '—' }}</span>
                </app-flex-table-cell>
                <app-flex-table-cell column="industry">
                  <span class="truncate text-muted-foreground">{{ company.industry || '—' }}</span>
                </app-flex-table-cell>
                <app-flex-table-cell column="contacts">
                  <span class="tabular-nums text-muted-foreground">{{ company.contactCount ?? 0 }}</span>
                </app-flex-table-cell>
                <app-flex-table-cell column="owner">
                  <span class="truncate text-muted-foreground">{{ company.owner?.email || '—' }}</span>
                </app-flex-table-cell>
              </app-flex-table-row>
            }
          </app-flex-table>
    ...
    loader: async ({ params, abortSignal }) => {
      ...
      return runResourceLoader(
        async () => {
          ...
          throwIfAborted(abortSignal);
          return { companies: result.data, total: result.total } satisfies CompaniesPageResult;
        },
